...
def showStores():
  ...
  if 'username' not in login_session:
      ...
      return render_template('stores.html', stores=stores, logged_in=('username' in login_session))
...
def newStore():
  ...
  if request.method == 'POST':
      ...
      return render_template('newStore.html', logged_in=('username' in login_session))
...
def editStore(store_id):
  ...
  if request.method == 'POST':
    ...
    return render_template('editStore.html', store = editedStore, logged_in=('username' in login_session))
...
def deleteStore(store_id):
  ...
  if request.method == 'POST':
      ...
      return render_template('deleteStore.html',store = storeToDelete, logged_in=('username' in login_session))
...
def showItem(store_id):
    ...
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('publicitems.html', items=items, store=store, creator=creator)
    else:
        return render_template('item.html', items=items, store=store, creator=creator, logged_in=('username' in login_session))
...
def newItem(store_id):
  ...
  if login_session['user_id'] != store.user_id:
      return "<script>function myFunction() {alert('You are not authorized to add fashion items to this store. Please create your own store in order to add items.');}</script><body onload='myFunction()''>"
  if request.method == 'POST':
      ...
      return render_template('newItem.html', store_id = store_id, logged_in=('username' in login_session))
...
def editItem(store_id, item_id):
    ...
    store = session.query(Store).filter_by(id=store_id).one()
    if login_session['user_id'] != store.user_id:
        return "<script>function myFunction() {alert('You are not authorized to edit fashion items to this store. Please create your own store in order to edit items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
        ...
        return render_template('editItem.html', store_id = store_id, item_id = item_id, item = editedItem, logged_in=('username' in login_session))
...
def deleteItem(store_id,item_id):
    ...
    itemToDelete = session.query(FashionItem).filter_by(id=item_id).one()
    if login_session['user_id'] != store.user_id:
        return "<script>function myFunction() {alert('You are not authorized to delete fashion items to this store. Please create your own store in order to delete items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        ...
        return render_template('deleteItem.html', item = itemToDelete, logged_in=('username' in login_session))
